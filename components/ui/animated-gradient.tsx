...
/**
 * Vendored WebGL2 animated-gradient primitive. Lives in components/ui
 * (rather than the app's own components/) because it's a generic,
 * app-agnostic visual primitive — no knowledge of this app's data —
 * the same convention shadcn/ui uses for "ui" vs feature components,
 * adopted here without pulling in the shadcn CLI/Radix stack, since
 * this component needs neither.
 *
 * Adapted from the pasted source with one change: the original's
 * `isMounted` state + effect (`useEffect(() => setIsMounted(true), [])`)
 * was removed — Client Component effects never run during SSR, so by
 * the time any effect fires here the component is already mounted on
 * the client; the flag was always true whenever it mattered. Dropping
 * it also sidesteps this repo's react-hooks/set-state-in-effect lint
